...
/*** defines for TCP *****/
#define TCP_ID "SCR"
#define TCP_DEFAULT_TIMEOUT 1000
...
wait_for_identification(int index)
{
    ...
    while (!identified)
    {
        ...
        while( recv(listenSocket[index], line, TCP_MSGLEN, 0) < 0 )
        {
            std::cerr << "Error: problem in receiving from the listen socket"<<std::endl;
            sleep(1);
        }
...
drive(int index, tCarElt* car, tSituation *s)
{
    ...
    if (RESTARTING[index]==0)
    {
        ...
        fcntl(listenSocket[index], F_SETFL, O_NONBLOCK); /* Change the socket into non-blocking state	*/
        ...
        if (sendto(listenSocket[index], line, 1000, 0,
                  (struct sockaddr *) &clientAddress[index],
                  sizeof(clientAddress[index])) < 0){
            //std::cerr << "Error: cannot send car state"<< std::endl;
            //sleep(1);
        }
...
//	tSockAddrIn im_addr = 10;
	int im_addr = 10;
        if (sendto(listenSocket[index], im_msg.c_str(), im_msg.size(), 0,
                   (struct sockaddr *) &im_addr,
                   sizeof(im_addr)) < 0){
            //std::cerr << "Error: cannot send car image";
            //sleep(1);
        }
*/        
        //std::cout << "TORCS server#"<<index<<" = Wait for Client To Receive Packet = "<<float( clock () - begin_time ) /  CLOCKS_PER_SEC * 1000 << " ms"<<std::endl;
        ...
        // Set to Non Blocking listenSocket
    ...
    //#endif
        ...
        //std::cout << "TORCS server#"<<index<<" = Wait for Client Answer = "<<float( clock () - begin_time ) /  CLOCKS_PER_SEC * 1000 << " ms"<<std::endl;
