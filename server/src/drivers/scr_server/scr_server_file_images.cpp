...
takeScreenShot()
{
	...
	if (GfCreateDir(path) == GF_DIR_CREATED) {
		...
		glReadPixels(0, 0, cam_width, cam_height, GL_RGB, GL_UNSIGNED_BYTE, (GLvoid*)img);
...
//		GfImgWritePng(img, "/home/linux/Work/Torcs/snap.png", cam_width, cam_height);
		GfImgWritePng(img, buf, cam_width, cam_height);
...
public:
    ...
    ~image_server(){
        std::cout<<"image server dtor"<<std::endl;
        delete cam_data;
        if(stream!=NULL)
            fclose (stream);
        stream = NULL;
    }
    void init(int id=3001){
        std::stringstream idstr;
        idstr << id;
        string path = "/tmp/ram" + idstr.str();
	...
	if(stream)
        	fwrite(cam_data, imgsize, 1, stream);

        if(debug)
            std::cout << "Writing Time = "<<float( clock () - begin_time ) /  CLOCKS_PER_SEC * 1000 << " ms"<<std::endl;
    }
